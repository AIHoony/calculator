...
class Calculator(QWidget):  # QWidget 클래스를 상속받아 Calculator 클래스 정의
    ...
    def initUI(self):
        ...
        self.te1.setReadOnly(True)  # 텍스트 에디트 위젯을 읽기 전용으로 설정 (사용자가 입력할 수 없도록)
        ...
        hbox = QHBoxLayout()    # 수평 레이아웃 위젯 생성하고 버튼 1, 2 추가
        hbox.addStretch(1) # 빈 공간
        hbox.addWidget(self.btn1)   # 메시지 버튼 배치
        hbox.addWidget(self.btn2)   # 클리어 버튼 배치

        vbox=QVBoxLayout()  # 수직 레이아웃 위젯 생성
        vbox.addWidget(self.te1)    # 수직 에이아웃에 텍스트 에디트 위젯 추가
...
if __name__ == '__main__':  # pyqt는 애플리케이션 당 1개의 QApplication 객체가 필요함
    app = QApplication(sys.argv)    # QApplication 인스턴스 생성
    ex = Calculator()   # Calculator 윈도 인스턴스 생성
